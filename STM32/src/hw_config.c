...
extern uint8_t  Tx_Buffer[]; // буфер передачи
uint32_t Tx_data_len = 0; // Длина передаваемых жанных
uint8_t  USB_Tx_State = 0; // Флаг разрешения передачи
...
void send_a_transaction(void)
{
  ...
  if ( USB_Tx_length ) {
    send_len = (USB_Tx_length < TX_BUFFER_SIZE ) ? USB_Tx_length : TX_BUFFER_SIZE;
